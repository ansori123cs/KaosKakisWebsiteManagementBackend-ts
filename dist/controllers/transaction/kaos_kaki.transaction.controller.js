        ...
        const result = await db.query.kaosKaki.findMany({
            ...
            with: {
                ...
                kaosKakiDetailMesins: {
                    columns: {},
                    where: (kaosKakiDetailMesin, { eq, and, isNull }) => and(eq(kaosKakiDetailMesin.isDeleted, false), isNull(kaosKakiDetailMesin.deletedAt)),
                    with: {
                        jenisMesin: {
                            columns: {
                                nama: true,
        ...
        await db.transaction(async (tx) => {
            ...
            if (payload.variasiDetail) {
                for (const variasi of payload.variasiDetail) {
                    const isExist = checkKaosKakiData.kaosKakiDetailVariasi.some((f) => f.id === variasi.id &&
